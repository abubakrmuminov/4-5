...
export function addFemalePerson(people, newPerson) {
  if (newPerson.gender !== "female") {
    console.log("Faqat ayol qo‘shish mumkin!");
    return people.slice(); // Original array nusxasini qaytarish
  }
  return people.concat([newPerson]);
}
const people6 = [
...
export function findByName(people, name) {
  ...
  if (!person) {
    console.log(`Ism ${name} topilmadi!`);
    return null;
  }
  return person;
}
...
export function findByGender(people, gender) {
  const matches = people.filter((person) => person.gender === gender);
  if (matches.length === 0) {
    console.log(`Jins ${gender} bo‘yicha hech kim topilmadi!`);
    return [];
  }
  return matches;
}
...
export function addPersonWithGenderCheck(people, newPerson) {
  if (newPerson.gender !== "male" && newPerson.gender !== "female") {
    console.log("Jins faqat 'male' yoki 'female' bo‘lishi mumkin!");
    return people.slice();
  }
  return people.concat([newPerson]);  
